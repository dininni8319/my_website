<section id="skills">
    <div class="skills-title">
        <h3>Skills</h3>
    </div>
    <div class="skills-details">
        <div class="speciality-details">
            <h4 class="h4-skills-details">Frontend</h4>
            <div>
                <i class="fab fa-html5 fa-2x html"></i>
                <i class="fab fa-css3 fa-2x css"></i>
                <i class="fab fa-js fa-2x js"></i>
                <i class="fab fa-bootstrap fa-2x bootstrap"></i>
                <i class="fab fa-react fa-2x react"></i>
            </div>
            <p class="p-skills-details">
             HTML5, CSS3, Bootstrap, Javascript, React.
            </p>
        </div>
        <div class="speciality-details class-shadow">
            <h4 class="h4-skills-details">Backend</h4>
            <div>
                <i class="fab fa-laravel fa-2x laravel"></i>
                <i class="fas fa-database fa-2x databases"></i>
                <i class="fab fa-php fa-2x php"></i>
            </div>
            <p class="p-skills-details">
              Laravel, PHP, MySql.
            </p>
        </div>
        <div class="speciality-details class-shadow">
            <h4 class="h4-skills-details">Version Control</h4>
            <div>
                <i class="fab fa-git-alt fa-2x git"></i>
                <i class="fab fa-gitlab fa-2x gitlab"></i> 
            </div>
            <p class="p-skills-details">
              Git, Gitlab, Github.
            </p>
        </div>
        <div class="speciality-details class-shadow">
            <h4 class="h4-skills-details">Agile methodologies</h4>
            <img src="/agile.jpeg" alt="" class="skills-img">
            <p class="p-skills-details">
              Agile, Scrum Framework.
            </p>
        </div>
        <div class="speciality-details class-shadow">
            <h4 class="h4-skills-details">Infrastructure</h4>
            <i class="fas fa-server fa-2x servers"></i>
             
            <p class="p-skills-details">
              Web Servers, Hosting.
            </p>
        </div>
    </div>
</section>